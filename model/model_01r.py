...
class DistLayer(nn.Module):
    def __init__(self, n_dist_embed, n_atom_embed):
        ...
        self.fc1 = nn.Linear(self.n_de + self.n_ae*2, self.n_ae)
        self.bn1 = nn.BatchNorm1d(self.n_ae*2)
        
    def forward(self, x, dist_feat, atom_idx, ele2_idx):
        x1 = torch.cat([
            x[..., :self.n_ae],
            global_max_pool(x[..., self.n_ae:], ele2_idx)[ele2_idx, :],
            dist_feat
        ], dim=1)

        x2 = torch.cat([
            global_max_pool(x[..., :self.n_ae], atom_idx)[atom_idx, :],
            x[..., self.n_ae:], 
            dist_feat
        ], dim=1)

        h1 = F.relu(self.fc1(x1))
        h2 = F.relu(self.fc1(x2))

        h = self.bn1(torch.cat([h1, h2], dim=1))
        out = F.relu(h + x)
        return out
...
class ModuleDistLayers(nn.Module):
    ...
    def forward(self, x, rdf_feat, bdf_feat, atom_idx, ele2_idx):
        x1 = self.rdf_layer(x, rdf_feat, atom_idx, ele2_idx)
        x2 = self.bdf_layer(x, bdf_feat, atom_idx, ele2_idx)
...
class DistNN(nn.Module):
    ...
    def forward(self, atom_feat, rdf_feat, bdf_feat, atom_idx, ele1_idx, ele2_idx, graph_idx, ref_feat):
        ...
        h = self.dist_layer_1(h, x_rdf, x_bdf, atom_idx, ele2_idx)
        h = self.dist_layer_2(h, x_rdf, x_bdf, atom_idx, ele2_idx)
