...
#        loss = criterion(batch[7], pred) + criterion(batch[8], mtype)
        ...
        loss1 = criterion(-torch.log(batch[8]/100 + 1e-4), pred)
        loss2 = criterion(batch[8], 100*torch.exp(-pred) - 0.01)
    ...
    with torch.no_grad():
        for batch in data_loader:
            ...
            if criterion is not None:
                loss1 = criterion(-torch.log(batch[8]/100 + 1e-4), pred).detach().item()
                loss2 = criterion(batch[8], 100*torch.exp(-pred) - 0.01).detach().item()
